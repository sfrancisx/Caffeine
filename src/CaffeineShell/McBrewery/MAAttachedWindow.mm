...
CGFloat getScaleFactor()
{
    ...
    if (NSAppKitVersionNumber >= NSAppKitVersionNumber10_7)
        ...
        scale = [[NSScreen mainScreen] userSpaceScaleFactor];
    
    return  scale;
}
 */

//#define MAATTACHEDWINDOW_SCALE_FACTOR getScaleFactor()
                        ...
                        atDistance:(float)distance
{
    ...
    if ((self = [super initWithContentRect:contentRect 
        ...
        _showRightCorner = TRUE;
        _showLeftCorner = TRUE;
...
- (void)_updateGeometry
{
    ...
    //NSPoint np = NSMakePoint(0, _point.y * MAATTACHEDWINDOW_SCALE_FACTOR);
    NSRect np;
    np.origin.x = 0;
    np.origin.y = _point.y * MAATTACHEDWINDOW_SCALE_FACTOR;
    np.size.width = contentRect.size.width;
    np.size.height = contentRect.size.height;
    
    ////contentRect.origin = (_window) ? [_window convertBaseToScreen:_point] : _point;
    //contentRect.origin = (_window) ? [_window convertBaseToScreen:np] : np;
    
    contentRect = (_window) ? [_window convertRectToScreen: np] : np;
...
- (MAWindowPosition)_bestSideForAutomaticPosition
{
    ...
    // convert rect, using non deprecated
    NSPoint pointOnScreen = _point;
    if ( _window )
    {
        NSRect rtPt = NSMakeRect(_point.x, _point.y, 1, 1);
        NSRect nPt = [_window convertRectToScreen:rtPt];
        pointOnScreen.x = nPt.origin.x;
        pointOnScreen.y = nPt.origin.y;
    }
...
- (void)showRightCorner:(BOOL) value
{
    if ( _showRightCorner != value )
    {
        _showRightCorner = value;
        [self _updateBackground];
    }
}

- (void)showLeftCorner:(BOOL) value
{
    if ( _showLeftCorner != value )
    {
        _showRightCorner = value;
        [self _updateBackground];
    }
...
- (void)removeParentWindowForCleaning
{
...
{
    // changed it from the dealloc to performClose
    [[NSNotificationCenter defaultCenter] removeObserver:self];
    
    if([[self delegate] respondsToSelector:@selector(windowWillClose:)])
    {
        YLog(LOG_ONLY_IN_DEBUG, @"performClose - MAAttachedWindow delegate responds to windowWillClose");
...
{
    YLog(LOG_ONLY_IN_DEBUG, @"Received a resize notification (MAAtachedWindow) note=%@", note);
    [self _redisplay];
}
...
- (void)displayChanged:(NSNotification *)note
{
    ...
    {
        YLog(LOG_ONLY_IN_DEBUG, @"Received notification of display change - calling _redisplay");
        [self _redisplay];
    }
}
