...
/**
 * A split tab control - the tabs are in the same window as the content they control.  Compare to
 * DockedTab.
 *
 * A window can only have one split tab.  For convenience, you can pass the tab definition
...
debugger;
	...
	if (!isCreated)
	{
		...
		parms.parent.innerHTML = '<div style="left:72px;position:relative"></div><div></div>';

		Caffeine.Ctrls.OverlayCtrl( { parent: parms.parent.firstChild, intf: intf }, function(err, ctrl)
			{
				overlayCtrl = ctrl;

				Caffeine.IPC.makeProxy(Caffeine.Ctrls.SplitTab, function(err, ctrl)
				{
					proxyCtrl = ctrl;
					...
					dockParms.parent = parms.parent.firstChild.nextSibling;
					dockParms.intf = intf;

					Caffeine.Ctrls.TileCtrl( dockParms, function(err, ctrl)
					{
						tabCtrl = ctrl;
